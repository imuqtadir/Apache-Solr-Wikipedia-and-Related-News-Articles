...
<script>
            function reload() {
                ...
                window.location.href = "./SearchHome.jsp?query=" + q;
            }

            function autocom() {
                if (document.getElementById("query").value.length > 5) {
                    $.get('http://localhost:8080/Solr_Web_Application/autocomplete?data=' + document.getElementById("query").value, function(data) {
                        //alert(data);
                    });
                }
            }
            ...
            function Convert_to_link(name, divid) {
                ...
                var content = document.getElementById(divid).value;
                ...
                // return name;
                $.post('http://localhost:8080/Solr_Web_Application/RelevanceServlet',{url:temp,contents:content,name:name},function(data){
										...
										<%
											if (request.getParameter("query") != null) {
												...
												try {
													...
													request.setAttribute("jsonList1", objwrapper.news_response);
												} catch (Exception ex) {
													ex.printStackTrace();
												}
									...
									<table style="width: 100%">
										...
										<tr>
											<td colspan="2">Did you mean <u><b><a
														href="http://localhost:8080/Solr_Web_Application/webcontent/SearchHome.jsp?query=<%=objwrapper.spellcheck_query%>"
													...
													<c:forEach var="jsonitem" items="${jsonList}">
														<div class="panel panel-info">
															<div class="panel-heading">
																<h3 class="panel-title">
																	<a style="cursor: pointer; font-weight: bold;"
																		onclick="Convert_to_link('${jsonitem.get('name').getAsString()}', '${jsonitem.get('id').getAsString()}')">
															...
															<div style="text-align: justify;" class="panel-body">
																...
																<input type="hidden"
																	id="${jsonitem.get('id').getAsString()}"
																	value="${jsonitem.get('contents').getAsString()}">
																	</div>
															</div>
														</div>
																	...
																	<div id="panel-element-82063" class="panel-collapse">
																		<c:forEach var="jsonitem1" items="${jsonList1}"
																			...
																			<div class="panel-body">
																				<a onclick="tolink('${jsonitem1.name}')" href="#">
																					<c:out value="${jsonitem1.name}" />
																				</a>
																			</div>
																		</c:forEach>
																	</div>
																		...
																		<c:forEach var="jsonitem"
																			...
																			<div class="panel-body">
																				<c:choose>
																					...
																					<c:otherwise>
																						<a target="_blank"
																							href="<c:out value="${jsonitem.source}" />"><c:out
